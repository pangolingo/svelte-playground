<script lang="ts">
  ...
  let name =
    (form?.errors?.values.name as string | null | undefined) ?? (data.session?.user?.name || '');
  let email =
    (form?.errors?.values.email as string | null | undefined) ?? (data.session?.user?.email || '');
