<script lang="ts">
	...
	data.streaming.favoritePokemon.then(f => f.map((p) => p.pokemon_id)).then(f => { favoritePokemonIds = f });
</script>
...
{:then value}
	...
	<div class="flex mt-2 space-x-2">
		{#if value.page.previous}<a href="?page={pageNumber - 1}" class="btn">Previous page</a
			>{/if}
